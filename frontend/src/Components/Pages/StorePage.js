...
async function getCurrentUser() {
  ...
  const responseUser = await fetch(`/api/users/user?username=${user.username}`);
...
// const options = {
  ...
  const responseAddingSkinToUser = await fetch(`/api/users/skins?username=${currentUser.username}`, options);
